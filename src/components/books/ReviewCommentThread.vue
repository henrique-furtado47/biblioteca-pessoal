...
const props = defineProps({ reviewId: { type: String, default: '' } })
...
async function load() {
  ...
  loading.value = true
  try {
    comments.value = await reviewsService.listComments(props.reviewId, auth.user.id)
...
async function likeComment(c) {
  ...
  try {
    await reviewsService.toggleCommentLike(c.id, auth.user.id, was)
  } catch {
    c.liked = was
    c.like_count += was ? 1 : -1
...
async function add(body, parentId = null) {
  ...
  busy.value = true
  try {
    const c = await reviewsService.addComment(props.reviewId, auth.user.id, text, parentId)
...
async function submitReply(comment) {
  await add(replyDraft.value, comment.parent_id || comment.id)
  replyDraft.value = ''
  replyTo.value = null
}
      ...
      <div v-for="r in repliesOf(c.id)" :key="r.id" class="ml-9 flex gap-2">
        ...
        <div>
          ...
          <button class="ml-3 mt-0.5 flex items-center gap-1 text-xs" :class="r.liked ? 'text-rose-500' : 'text-slate-400 hover:text-rose-500'" @click="likeComment(r)">
            <svg class="h-3.5 w-3.5" :class="r.liked ? 'fill-rose-500' : 'fill-none'" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/></svg>
